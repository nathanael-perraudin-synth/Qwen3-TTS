...
def copy_model_weights(source_model, target_model):
    ...
    with torch.no_grad():
        # Copy speaker encoder if exists
        if hasattr(source_model, 'speaker_encoder') and source_model.speaker_encoder is not None:
            if hasattr(target_model, 'speaker_encoder') and target_model.speaker_encoder is not None:
                try:
                    target_model.speaker_encoder.load_state_dict(
                        source_model.speaker_encoder.state_dict(), strict=False
                    )
                except Exception as e:
                    print(f"Warning: Could not copy speaker encoder weights: {e}")
        
        # Copy talker model
        if hasattr(source_model, 'talker') and hasattr(target_model, 'talker'):
            # Copy talker model (core transformer)
            try:
                target_model.talker.model.load_state_dict(
                    source_model.talker.model.state_dict(), strict=False
                )
            except Exception as e:
                print(f"Warning: Could not copy talker model weights: {e}")
            
            # Copy text projection
            try:
                target_model.talker.text_projection.load_state_dict(
                    source_model.talker.text_projection.state_dict(), strict=False
                )
            except Exception as e:
                print(f"Warning: Could not copy text projection weights: {e}")
            
            # Copy codec head
            try:
                if target_model.talker.codec_head.weight.shape == source_model.talker.codec_head.weight.shape:
                    target_model.talker.codec_head.weight.data.copy_(
                        source_model.talker.codec_head.weight.data
                    )
            except Exception as e:
                print(f"Warning: Could not copy codec head weights: {e}")
            
            # Copy code predictor
            if hasattr(source_model.talker, 'code_predictor') and hasattr(target_model.talker, 'code_predictor'):
                # Copy code predictor model
                try:
                    target_model.talker.code_predictor.model.load_state_dict(
                        source_model.talker.code_predictor.model.state_dict(), strict=False
                    )
                except Exception as e:
                    print(f"Warning: Could not copy code predictor model weights: {e}")
                
                # Copy lm_head
                try:
                    min_len = min(
                        len(target_model.talker.code_predictor.lm_head),
                        len(source_model.talker.code_predictor.lm_head)
                    )
                    for i in range(min_len):
                        if (target_model.talker.code_predictor.lm_head[i].weight.shape == 
                            source_model.talker.code_predictor.lm_head[i].weight.shape):
                            target_model.talker.code_predictor.lm_head[i].weight.data.copy_(
                                source_model.talker.code_predictor.lm_head[i].weight.data
                            )
                except Exception as e:
                    print(f"Warning: Could not copy code predictor lm_head weights: {e}")
                
                # Copy projection if exists
                if hasattr(target_model.talker.code_predictor, 'small_to_mtp_projection'):
                    if hasattr(source_model.talker.code_predictor, 'small_to_mtp_projection'):
                        try:
                            target_model.talker.code_predictor.small_to_mtp_projection.load_state_dict(
                                source_model.talker.code_predictor.small_to_mtp_projection.state_dict(),
                                strict=False
                            )
                        except Exception as e:
                            print(f"Warning: Could not copy small_to_mtp_projection weights: {e}")
