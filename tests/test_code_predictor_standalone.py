...
from qwen_tts.core.models.standalone_config import StandaloneCodePredictorConfig
...
def standalone_config():
    """Create standalone code predictor config."""
...
def test_models_produce_identical_outputs(original_model, standalone_model, sample_inputs_embeds, sample_position_ids):
    ...
    with torch.no_grad():
        # Copy embeddings
        for orig_emb, stand_emb in zip(original_model.codec_embedding, standalone_model.codec_embedding):
            stand_emb.weight.data.copy_(orig_emb.weight.data)
