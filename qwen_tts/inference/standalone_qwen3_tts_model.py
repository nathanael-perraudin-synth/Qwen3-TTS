...
"""
Standalone version of Qwen3TTSModel without transformers dependency.
This module provides the same API as qwen3_tts_model.py but uses standalone models.
"""
import base64
import io
import json
...
from ..core.models.standalone_top_level import StandaloneQwen3TTSForConditionalGeneration
...
class VoiceClonePromptItem:
    ...
    Fields are aligned with `StandaloneQwen3TTSForConditionalGeneration.generate(..., voice_clone_prompt=...)`.
    """
...
class StandaloneQwen3TTSModel:
    """
    Standalone wrapper for Qwen3 TTS models (CustomVoice/VoiceDesign/Base) that provides:
      - from_pretrained() initialization without transformers dependency
      - generation APIs for:
    ...
    Notes:
      - This wrapper expects the underlying model class to be `StandaloneQwen3TTSForConditionalGeneration`
      - Language / speaker validation is done via model methods:
          model.get_supported_languages(), model.get_supported_speakers()
      - Processor (tokenizer) is still required but can be loaded separately
    """

    def __init__(
        self,
        model: StandaloneQwen3TTSForConditionalGeneration,
        processor,
    ...
    def from_pretrained(
        ...
        processor=None,
        **kwargs,
    ) -> "StandaloneQwen3TTSModel":
        """
        Load a Qwen3 TTS model and its processor without transformers dependency.

        This method:
          1) Loads config from config.json
          2) Creates standalone model instance
          3) Loads weights from state_dict
          4) Loads processor (tokenizer) if provided
          5) Loads optional `generate_config.json` from the model directory/repo snapshot if present.

        Args:
            pretrained_model_name_or_path (str):
                HuggingFace repo id or local directory of the model.
            processor:
                Optional processor (tokenizer). If None, must be loaded separately.
            **kwargs:
                Additional arguments for model initialization (device_map, dtype, etc.).

        Returns:
            StandaloneQwen3TTSModel:
                Wrapper instance containing `model`, `processor`, and generation defaults.
        """
        # Bridge: load the original (transformers) model, then build standalone
        # and copy weights so the returned object is a StandaloneQwen3TTSModel.
        ...
        device = torch.device(device_map if isinstance(device_map, str) else "cuda:0")

        original_wrapper = Qwen3TTSModel.from_pretrained(
        ...
        original_model = original_wrapper.model
        ...
        with torch.no_grad():
            if original_model.speaker_encoder is not None and standalone_model.speaker_encoder is not None:
                try:
                    standalone_model.speaker_encoder.load_state_dict(
                        original_model.speaker_encoder.state_dict(), strict=False
                    )
                except Exception as e:
                    raise RuntimeError(f"Failed to copy speaker encoder weights: {e}") from e
            try:
                standalone_model.talker.load_state_dict(
                    original_model.talker.state_dict(), strict=False
                )
            except Exception as e:
                raise RuntimeError(f"Failed to copy talker weights: {e}") from e

        standalone_model.speech_tokenizer = original_model.speech_tokenizer
        processor = processor if processor is not None else original_wrapper.processor
        generate_defaults = dict(original_wrapper.generate_defaults or {})
    ...
    def generate_voice_clone(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs:
                Any other keyword arguments supported by the model's `generate()` can be passed.
